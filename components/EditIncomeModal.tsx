...
export default function EditIncomeModal({ income, onClose, onSaved }: Props) {
            ...
            <div className="w-full max-w-md bg-[#0e0e1a] border border-[#1e1e35] rounded-2xl shadow-2xl slide-up" onClick={e => e.stopPropagation()}>
                <div className="flex items-center justify-between px-6 py-5 border-b border-[#1e1e35]">
                    <div className="flex items-center gap-3">
                        ...
                        <h2 className="text-base font-bold text-white">Editar Recebimento</h2>
                    </div>
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-4">
                    ...
                    <div>
                        ...
                        <input value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} required
                            className="w-full bg-[#07070d] border border-[#1e1e35] rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:border-amber-500/50 transition-colors" />
                    </div>
                    ...
                    <div>
                        ...
                        <input value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} placeholder="Opcional"
                            className="w-full bg-[#07070d] border border-[#1e1e35] rounded-xl px-4 py-2.5 text-sm text-white placeholder-slate-600 focus:outline-none focus:border-amber-500/50 transition-colors" />
                    </div>
                    ...
                    <div className="flex gap-3 pt-2">
                        ...
                        <button type="submit" disabled={loading}
                            className="flex-1 py-2.5 text-sm font-semibold text-white bg-amber-600 hover:bg-amber-500 rounded-xl transition-all hover:shadow-lg hover:shadow-amber-500/25 active:scale-[0.98] disabled:opacity-50">
