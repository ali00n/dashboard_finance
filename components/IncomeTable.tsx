...
function DeleteModal({ income, onConfirm, onCancel }: { income: Income; onConfirm: () => void; onCancel: () => void }) {
    return (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center modal-overlay p-4" onClick={onCancel}>
    ...
    if (incomes.length === 0) {
        return (
            <div className="bg-[#0e0e1a] border border-[#1e1e35] rounded-2xl flex flex-col items-center justify-center py-12 sm:py-16 text-center">
                ...
                <p className="text-slate-400 font-medium text-sm sm:text-base">Nenhum recebimento registrado</p>
                <p className="text-slate-600 text-xs sm:text-sm mt-1">Clique em &quot;Novo Recebimento&quot; para começar</p>
            </div>
        );
    }
